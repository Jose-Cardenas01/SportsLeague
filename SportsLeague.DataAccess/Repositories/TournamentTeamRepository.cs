...
namespace SportsLeague.DataAccess.Repositories
{
...

    {
        ...
        {
            return await _context.Set<TournamentTeam>().Where(tt => tt.TournamentId == tournamentId && tt.TeamId == teamId)
                                                       .FirstOrDefaultAsync();
        }

        public async Task<IEnumerable<TournamentTeam>> GetByTournamentAsync(int tournamentId)
        {
            return await _context.Set<TournamentTeam>().Where(tt => tt.TournamentId == tournamentId)
                                                       .Include(tt => tt.Team)
                                                       .ToListAsync();
        }
    }
