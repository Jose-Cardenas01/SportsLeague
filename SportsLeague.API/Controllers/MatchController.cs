...
public class MatchController : ControllerBase
{
    ...
    public async Task<ActionResult<MatchResponseDTO>> GetById(int id)
    {
        ...
        {
            return NotFound(
                new { message = $"Partido con ID {id} no encontrado" });
        }
    ...
    public async Task<ActionResult<MatchResponseDTO>> Create(MatchRequestDTO dto)
    {
        try
        {
            ...
            var matchWithDetails =
                await _matchService.GetByIdAsync(created.Id);

            var responseDto =
                _mapper.Map<MatchResponseDTO>(matchWithDetails);
