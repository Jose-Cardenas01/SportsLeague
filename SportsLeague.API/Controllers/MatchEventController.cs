...
public class MatchEventController : ControllerBase
{
        ...
        MatchResultRequestDTO dto)
    {
        try
        {
            ...
            var created =
                await _matchEventService.RegisterResultAsync(
                    matchId,
                    result);

            return Ok(
                _mapper.Map<MatchResultResponseDTO>(created));
        }
    ...
    [HttpGet("result")]
    public async Task<ActionResult<MatchResultResponseDTO>> GetResult(
        int matchId)
    {
        try
        {
            var result =
                await _matchEventService.GetResultByMatchAsync(matchId);
            ...
            if (result == null)
            {
                return NotFound(
                    new
                    {
                        message = "Este partido aún no tiene resultado"
                    });
            }

            return Ok(
                _mapper.Map<MatchResultResponseDTO>(result));
        }
        catch (KeyNotFoundException ex)
        {
            return NotFound(new { message = ex.Message });
        ...
        GoalRequestDTO dto)
    {
        try
        {
            ...
            var created =
                await _matchEventService.RegisterGoalAsync(
                    matchId,
                    goal);

            var goals =
                await _matchEventService.GetGoalsByMatchAsync(matchId);

            var createdGoal =
                goals.FirstOrDefault(g => g.Id == created.Id);

            return Ok(
                _mapper.Map<GoalResponseDTO>(createdGoal));
        }
        catch (KeyNotFoundException ex)
        {
            return NotFound(new { message = ex.Message });
        ...
        int matchId)
    {
        try
        {
            ...
            return Ok(
                _mapper.Map<IEnumerable<GoalResponseDTO>>(goals));
        }
        catch (KeyNotFoundException ex)
        {
            return NotFound(new { message = ex.Message });
        ...
        CardRequestDTO dto)
    {
        try
        {
            ...
            var created =
                await _matchEventService.RegisterCardAsync(
                    matchId,
                    card);

            var cards =
                await _matchEventService.GetCardsByMatchAsync(matchId);

            var createdCard =
                cards.FirstOrDefault(c => c.Id == created.Id);

            return Ok(
                _mapper.Map<CardResponseDTO>(createdCard));
        }
        catch (KeyNotFoundException ex)
        {
            return NotFound(new { message = ex.Message });
        ...
        int matchId)
    {
        try
        {
            ...
            return Ok(
                _mapper.Map<IEnumerable<CardResponseDTO>>(cards));
        }
        catch (KeyNotFoundException ex)
        {
            return NotFound(new { message = ex.Message });
