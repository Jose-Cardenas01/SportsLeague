...
public class MatchService : IMatchService
{
    ...
    public async Task<Match> CreateAsync(Match match)
    {
        ...
        // 3. Validar equipos
        var homeTeamExists = await _teamRepository.ExistAsync(match.HomeTeamId);
        ...
        var awayTeamExists = await _teamRepository.ExistAsync(match.AwayTeamId);
        ...
        // 5. Validar árbitro
        var refereeExists = await _refereeRepository.ExistAsync(match.RefereeId);
    ...
    public async Task UpdateStatusAsync(int id, MatchStatus newStatus)
    {
        ...
        _logger.LogInformation(
            "Updating match {MatchId} status to {NewStatus}",
            id,
            newStatus);
